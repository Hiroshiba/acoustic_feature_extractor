...
def process(
    ...
    if remove_all_same:
        datas = {
            ...
            if not data.all_same()
        }

    if padding:
        padding_value = base.estimate_padding_value()
        items = datas.items()
        indexes, datas = list(map(itemgetter(0), items)), list(map(itemgetter(1), items))
        datas = SamplingData.padding(datas, padding_value=padding_value)
        datas = {
            key: data
            for key, data in zip(indexes, datas)
        }

    for i, data in datas.items():
        if data is None:
            continue
        out = output_directory / file_format.format(stem=path.stem, suffix=path.suffix, i=i)
        data.save(out)
...
def main():
    ...
    parser.add_argument('--keypoint_seconds', '-ks', nargs='+', type=float, required=True)

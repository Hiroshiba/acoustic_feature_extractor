...
def process(
    ...
    f0 = F0.load(path=path).convert(
        input_statistics=load_numpy_object(input_statistics) if input_statistics is not None else None,
        target_statistics=load_numpy_object(target_statistics) if target_statistics is not None else None,

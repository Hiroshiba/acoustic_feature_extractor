...
class MidiFeature(object):
    ...
    def _make_note_array(self):
        ...
        array = numpy.zeros((len(self.notes), dim), dtype=numpy.float32)

        for i, note in enumerate(self.notes):
            assert self.pitch_range[0] <= note.pitch <= self.pitch_range[1]

            feature = numpy.zeros(dim, dtype=bool)
    ...
    def make_array(self, with_position=True):
        ...
        array = numpy.zeros((self.len_array, dim), dtype=numpy.float32)
        for i, note in enumerate(self.notes):
            s = self._to_index(note.start)
            e = self._to_index(note.end)

            features = [numpy.repeat(note_array[i].reshape(1, -1), repeats=e - s + 1, axis=0)]

            if with_position:
                pos_start = (self._to_time(numpy.arange(s, e + 1)) - note.start).reshape(-1, 1)

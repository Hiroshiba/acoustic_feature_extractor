...
def test_f0_convert(with_vuv: bool,):
    frequency = numpy.array([0, 100, 200, 300], dtype=numpy.float32)

    f0 = F0.from_frequency(
        frequency=frequency, frame_period=5, with_vuv=with_vuv,
    ).convert(
        input_mean=numpy.log(1), input_var=1, target_mean=numpy.log(2), target_var=1,
    )
    ...
    numpy.testing.assert_allclose(
        converted_frequency, expected_frequency, rtol=0, atol=1e-4
    )
